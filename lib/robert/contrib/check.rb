...
defn check.cpu_load do
  ...
  body {
    uptime_str = with_capistrano { capture(var[:cmd]) }
  ...
  spec {
    before do
      @action.should_receive(:with_capistrano).and_yield
    end
...
defn check.disk_usage do
  ...
  body {
    sel_fs = with_capistrano { capture(var[:cmd]) }.split($/)[1..-1].map do |l|
      tokens = l.split(/\s+/)
...
defn check.procs do
  ...
  body {
    sel_procs = with_capistrano { capture(var[:cmd]) }.split($/)[1..-1].select { |l| l =~ var[:regexp] }
    perf_data = {:count =>
