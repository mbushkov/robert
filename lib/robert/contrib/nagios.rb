...
nagios_commands_mappings.each do |act_name,nagios_name|
  conf "nagios_#{act_name}_service" do
    var[:nagios,:configuration,act_name.to_sym,:host] = conf_name
    ...
    act[:configure_nagios] = nagios.check(act[:configure_nagios]) {
      var(:host) { var[:nagios,:configuration,act_name.to_sym,:host] }
...
#conf :nagios_http_service do
#  var(:nagios,:configuration,:http,:host) { var[:production,:url] }
#end
