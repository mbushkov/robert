...
describe CLI do
  context "parse_args" do
    ...
    it "numbers are converted from string to numeric representation in variable assignments" do
      res = CLI.parse_args(["update", "verbose=4", "context=project1,0,something"], nil)
  ...
  context "args_to_rules" do
    ...
    it "creates rule :cmdline,:cmd for the first argument (the command)" do
      CLI.args_to_rules({:vars => {},
                          :command => "update"},
                        @conf)

      @conf.extend(Robert::RulesEvaluator) #TODO: get rid of this - it looks like a hack - too many implementation details
    ...
    it "creates rule with context :cmdline,:args,[index] for every positional argument, except the first one (the command)" do
      CLI.args_to_rules({:vars => {:force => "now"},
                          :cmd => "update"},
                        @conf)

      @conf.extend(Robert::RulesEvaluator) #TODO: get rid of this - it looks like a hack - too many implementation details
      @conf.var[:cmdline,:args,0].should == [:force, "now"]
      @conf.var[:cmdline,:args,:force].should == "now"
    end

    it "creates alias :cmdline,:args,:names for first positional argument" do
      CLI.args_to_rules({:vars => {[:project1,:project2,:project3] => true},
                          :cmd => "update"},
                        @conf)

      @conf.extend(Robert::RulesEvaluator) #TODO: get rid of this - it looks like a hack - too many implementation details
      @conf.var[:cmdline,:names].should == [:project1,:project2,:project3]
    end

    it ":cmdline,:args,:names contains an array even when only one name was passed as an argument" do
      CLI.args_to_rules({:vars => {:project1 => true},
                          :cmd => "update"},
                        @conf)

      @conf.extend(Robert::RulesEvaluator) #TODO: get rid of this - it looks like a hack - too many implementation details
      @conf.var[:cmdline,:names].should == [:project1]
    end

    it ":cmdline,:args,:names expand to all confs_names" do
      @conf.should_receive(:confs_names).and_return([:project1, :project2])
      
      CLI.args_to_rules({:vars => {:all => true},
                          :cmd => "update"},
                        @conf)

      @conf.extend(Robert::RulesEvaluator) #TODO: get rid of this - it looks like a hack - too many implementation details
      @conf.var[:cmdline,:names].should == [:project1, :project2]
    end
  end
end
